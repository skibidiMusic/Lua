...
local ImGui = {
	...
	UIAssetId = "rbxassetid://71057212480309"
}
...
local function setConfigFlag(self: any, configSaveData)
    if configSaveData and self.saveFlag then
        configSaveData.configIds[self.saveFlag] = self 
    end
end
...
function ImGui:ContainerClass(Frame: Frame, Class, Window, configSaveData)
	...
	function ContainerClass:Keybind(Config)
		...
		function Config:SetValue(NewKey: Enum.KeyCode | string)
...
function ImGui:CreateWindow(WindowConfig)
    ...
    function WindowConfig:CreateConfigSaveHandler(configPath: string)
        ...
        local function saveConfig(name: string)
            local config = {}
            for flag, element in configHandler.configIds do
                config[flag] = element:GetValue()
            end
            saveFolder:save(name, config)
        end 
        ...
        -->> UI
        local tab = WindowConfig:CreateTab({
            Name = "Configs",
            Visible = false 
        })
	...
	function WindowConfig:CreateTab(Config)
		...
		--// Apply animations
		Config = ImGui:ContainerClass(Content, Config, Window, configHandler)
...
local GuiParent = gethui() or PlayerGui
